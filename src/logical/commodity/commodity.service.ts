...
export class CommodityService {
  ...
  async createCommodity(body: any, username: string): Promise<any> {
    const {name, description = '', marketPrice = 0, saleMoney = 0,img } = body;
    const createCommoditySQL = `
      INSERT INTO commodity
        (ccolumn_id, commodity_name, commodity_desc, market_price, sale_money,img c_by)
      VALUES
        ('${uuid()}', '${name}', '${description}', ${marketPrice}, ${saleMoney},${img},'${username}');
    `;
      if(!name){
        return {
          code: 400,
